...
import random
random.seed(42)
type_pattern = re.compile("^\$.+(lit\$|:)")

in_dir = "outputs-project-bigger/"
in_dir = "outputs-project/"
min_source_vocab = 3
min_target_vocab = 10
minibatchMaxSize = 1000
...
file_count = 0
with open("test_projects.txt", "w") as f:
  for file in os.listdir(in_dir):
    if "DefinitelyTyped" in file:
      continue
    if os.stat(in_dir + "/" + file).st_size == 0:
...
target_words.append(("$UNKNOWN$", 0))
target_word_vocab = set([word for word, _ in target_words])

with open("source.wl", "w", encoding="utf-8") as out:
  for name, count in source_words:
    out.write(name)
    out.write("\n")

with open("target.wl", "w", encoding="utf-8") as out:
...
def write(file, sources, targets):
  with open(file, "w", encoding="utf-8") as f:
    ...
    for i in range(len(sources)):
      ...
      target_tokens = [token if token in target_word_vocab else '$UNKNOWN$' for token in target]
...
train_file = "seq2seq-train.txt"
valid_file = "seq2seq-valid.txt"
test_file = "seq2seq-test.txt"
